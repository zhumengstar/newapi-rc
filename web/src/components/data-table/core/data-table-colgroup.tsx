...
export function DataTableColgroup<TData>({
  ...
  const isResizing = table.options.enableColumnResizing === true
  const sizedColumns = isResizing
    ? columns
    : columns.filter((column) => !isContentSizedColumn(column.id))
...
function getColumnWidth<TData>(
  ...
  if (table.options.enableColumnResizing === true) {
    ...
    if (columnId === 'name' && !columnSizing[columnId]) {
      return undefined
    }
